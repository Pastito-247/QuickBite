...
const Menu = () => {
  ...
  const details = restaurantDetails[parseInt(id)] || {
    name: restaurantName || 'Nuestro Menú',
    type: 'Comida Rápida',
    rating: 4.7,
    time: '20-30 min',
    deliveryFee: 'Gratis',
    gradient: 'from-primary-600 via-orange-500 to-primary-700'
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (id && filteredData.length === 0) {
          const fallbackData = getFallbackMenu(id);
  ...
  const validateMenuStock = async (items) => {
    ...
    for (const item of items) {
      try {
        if (item.id < 100) {
          const response = await fetch(`http://localhost:8080/api/menu/${item.id}/validate-stock?quantity=1`);
          if (response.ok) {
            const data = await response.json();
