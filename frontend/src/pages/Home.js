...
const Home = () => {
  ...
  const popularRestaurants = mockRestaurants.filter(r => r.rating >= 4.7);
  const freeDeliveryRestaurants = mockRestaurants.filter(r => r.deliveryFee === 'Gratis');
  const fastRestaurants = mockRestaurants.filter(r => parseInt(r.time) <= 20);
